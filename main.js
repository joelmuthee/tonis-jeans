...
(async function() {
  ...
  function render() {
    ...
    filterMeta.textContent = `Showing ${shown} of ${filtered.length} · ${availCount} available`;
    ...
    // View-more button
    const moreWrap = document.getElementById('viewMoreWrap');
    if (moreWrap) moreWrap.remove();
    if (hasMore) {
      const wrap = document.createElement('div');
      wrap.id = 'viewMoreWrap';
      wrap.className = 'view-more-wrap';
      wrap.innerHTML = `<button class="btn-view-more" id="viewMoreBtn">View more (${filtered.length - visibleCount} more)</button>`;
